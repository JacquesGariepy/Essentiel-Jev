import { mimeSubject, AppError, fail, enc, text, safeLink, plainHTML, extractAddress, iso, zonedInstant, hash } from './common.mjs';
...
function gmailText(payload) {
  const plain=[],html=[];
  function walk(part,depth=0){if(!part||depth>20)return;if(part.body?.data){let s=Buffer.from(part.body.data,'base64url').toString('utf8');if(part.mimeType==='text/plain')plain.push(s);else if(part.mimeType==='text/html')html.push(s);}for(const p of (part.parts||[]).slice(0,100))walk(p,depth+1);}
  walk(payload);return plain.length?plain.join('\n'):plainHTML(html.join('\n'));
}
export function normalizeMessage(p,m,account,full=false) {
  ...
  const body=p==='google'?(full?gmailText(m.payload):m.snippet||''):(full?(m.body?.contentType?.toLowerCase()==='html'?plainHTML(m.body.content):m.body?.content||m.bodyPreview):m.bodyPreview)||'';
  ...
  record.sourceHash=hash({id:record.id,title:record.title,from:record.from,text:record.text,receivedAt:record.receivedAt});return record;
}
