...
export class ConnectedService {
  constructor({filename,env,fetchImpl}={}) {
    ...
    // Sanitized HTML of recently opened messages for the formatted view: memory only, never in the vault or the status payload.
    this.mailHtml=new Map();
  }
  ...
  async manageVault(action,password){
    ...
    try{if(action==='unlock')await this.vault.unlock(password);else if(action==='lock'){await this.vault.lock();this.mailHtml.clear();}else if(action==='erase'){await this.vault.erase();this.mailHtml.clear();}else fail('INVALID_ACTION','Unknown vault action.');return this.status();}
    finally{this.vaultBusy=false;}
  }
  async disconnect(p){providerId(p);if(this.active)fail('BUSY','A connector operation is still in progress.',409);const d=this.vault.state();delete d.accounts[p];if(d.diagnostics)delete d.diagnostics[p];delete d.snapshots[p];delete d.selections[p];d.actions=d.actions.filter(a=>a.provider!==p&&a.source?.provider!==p);for(const k of [...this.mailHtml.keys()])if(k.startsWith(p+':'))this.mailHtml.delete(k);this.oauth.cancel();await this.vault.save();return {disconnected:true,revoked:false,note:'Local credentials and cached data removed. Remove app access at the provider to revoke the OAuth grant.'};}
  ...
  async message(p,id){providerId(p);const s=this.vault.state().snapshots[p];const index=s?.messages.findIndex(m=>m.id===id);if(index===undefined||index<0)fail('SOURCE_NOT_FOUND','Choose a message returned by synchronization.',404);const m=await this.providers.message(p,id),key=p+':'+id;m.hasHtml=Boolean(m.html);this.mailHtml.delete(key);if(m.html){this.mailHtml.set(key,m.html);while(this.mailHtml.size>30)this.mailHtml.delete(this.mailHtml.keys().next().value);}s.messages[index]=m;await this.vault.save();return m;}
  /** Sanitized HTML for the formatted view of an opened message, or null. Requires an unlocked vault. */
  mailView(p,id){providerId(p);this.vault.state();return this.mailHtml.get(p+':'+String(id))||null;}
